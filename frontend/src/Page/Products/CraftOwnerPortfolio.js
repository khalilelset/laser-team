...
const CraftOwnerPortfolio = ({ productId }) => {
  ...
  useEffect(() => {
    const fetchPortfolio = async () => {
      try {
        const response = await axios.get(`http://localhost:4000/api/craftowner/portfolio/productid/6490208a2163278cc6ecab1a`);
...
<h1 className="text-center my-4">Portfolio of craft owner</h1>
  ...
  <Row className="text-left" style={{marginTop:"50px", marginLeft:'20px'}}>
    ...
    <h1 className="special-header" style={{color:"gray"}}>
        Hello MY Name <span className="text-primary" style={{fontSize:"40px"}}>{portfolio.ownerFName} {portfolio.ownerLName}</span>
      </h1>
      <div className="my-4">
